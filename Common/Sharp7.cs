...
namespace Sharp7
{
    ...
    class MsgSocket
    {
        ...
        public bool IsConnected { get => tcpSocket != null && tcpSocket.Connected; }
        ...
        private void CreateSocket()
        {
            ...
            connectEventArgs.Completed += AsyncEventCompleted;
            receiveEventArgs.Completed += AsyncEventCompleted;
            sendEventArgs.Completed += AsyncEventCompleted;
        }
    ...
    public class S7Client
    {
        ...
        private readonly static int Size_RD = 31; // Header Size when Reading 
        private readonly static int Size_WR = 35; // Header Size when Writing
        ...
        readonly byte[] S7_LIST_BLOCKS_OF_TYPE = {
            0x03, 0x00, 0x00, 0x1f,
            0x02, 0xf0, 0x80, 0x32,
            0x07, 0x00, 0x00, 0x00,
            0x00, 0x00, 0x08, 0x00,
        ...
        public int Connect()
        {
            ...
            if (!Connected
                && TCPConnect() == 0        // First stage : TCP Connection
                    && ISOConnect() == 0)   // Second stage : ISOTCP (ISO 8073) Connection
            {
        ...
        public bool Connected { get => (Socket != null) && (Socket.IsConnected); }
        ...
        public class CommentForces
        {
            ...
            private List<ForceJob> AddCommentToForce(List<ForceJob> forceringen, List<SymbolTableRecord> symbolTable)
            {
                ...
                foreach (ForceJob force in forceringen)
                {
                    ...
                    var found = symbolTable.FirstOrDefault(s => s.Address == force.FullAdress);
                    var commentedforce = force;
        ...
        readonly byte[] S7_DrvRW = { // 31-35 bytes
        0x03,0x00,
        0x00,0x1f,       // Telegram Length (Data Size + 31 or 35)
        0x02,0xf0, 0x80, // COTP (see above for info)
        0x32,            // S7 Protocol ID 
        ...
        readonly byte[] S7Drv_MRD_HEADER = {
        0x03,0x00,
        0x00,0x1f,       // Telegram Length (Data Size + 31 or 35)
        0x02,0xf0, 0x80, // COTP (see above for info)
        0x32,            // S7 Protocol ID 
        ...
        readonly byte[] S7Drv_MWR_HEADER = {
        0x03,0x00,
        0x00,0x1f,       // Telegram Length (Data Size + 31 or 35)
        0x02,0xf0, 0x80, // COTP (see above for info)
        0x32,            // S7 Protocol ID 
        ...
        private readonly static int Size_NckRD = 29; // Header Size when Reading 
        // !Never Used! private readonly static int Size_NckWR = 33; // Header Size when Writing 

        // S7 NCK Read/Write Request Header (contains also ISO Header and COTP Header)
        readonly byte[] S7_NckRW = { // 31-35 bytes
            0x03,0x00,
        ...
        readonly byte[] S7Nck_MRD_HEADER = {
            0x03,0x00,
            0x00,0x1d,       // Telegram Length (Data Size + 29 or 33)
            0x02,0xf0, 0x80, // COTP (see above for info)
            0x32,            // S7 Protocol ID 
        ...
        readonly byte[] S7Nck_MWR_HEADER = {
            0x03,0x00,
            0x00,0x1d,       // Telegram Length (Data Size + 29 or 33)
            0x02,0xf0, 0x80, // COTP (see above for info)
            0x32,            // S7 Protocol ID 
    ...
    public class S7DrvMultiVar
    {
        ...
        private readonly S7Client FClient;
        private readonly GCHandle[] Handles = new GCHandle[S7Client.MaxVars];
        private readonly S7Client.S7DrvDataItem[] DrvItems = new S7Client.S7DrvDataItem[S7Client.MaxVars];
        private int Count = 0;

        public int[] Results = new int[S7Client.MaxVars];

        // Adapt WordLength
        private bool AdjustWordLength(int WordLen)
        {
            // Calc Word size          
            int WordSize = S7.DataSizeByte(WordLen);
        ...
        public bool DrvAdd<T>(int DONumber, int ParameterNumber, int WordLen, int Start, int Amount, ref T[] Buffer, int Offset)
        {
            ...
            if (Count < S7Client.MaxVars)
            {
                //Syntax-ID for DriveES-Communication (int DrvSynID = 162)

                if (AdjustWordLength(WordLen))
                {
    ...
    public static class S7Drv
    {
        ...
        // !Never Used! private static readonly Int64 bias = 621355968000000000; // "decimicros" between 0001-01-01 00:00:00 and 1970-01-01 00:00:00
    ...
    public class S7NckMultiVar
    {
        ...
        private readonly S7Client FClient;
        private readonly GCHandle[] Handles = new GCHandle[S7Client.MaxVars];
        private readonly S7Client.S7NckDataItem[] NckItems = new S7Client.S7NckDataItem[S7Client.MaxVars];
        private int Count = 0;

        public int[] Results = new int[S7Client.MaxVars];

        // Adapt WordLength
        private bool AdjustWordLength(int WordLen, ref int Amount)
        {
            // Calc Word size          
        ...
        public bool NckAdd<T>(int NckArea, int NckUnit, int NckModule, int ParameterNumber, int WordLen, int Start, int Amount, ref T[] Buffer, int Offset)
        {
            if (Count < S7Client.MaxVars)
            {
                //Syntax-ID for Nck-Communication (int NckSynID = 130)

                if (AdjustWordLength(WordLen, ref Amount))
                {
    ...
    public static class S7Nck
    {
        ...
        // !Never Used! private static Int64 bias = 621355968000000000; // "decimicros" between 0001-01-01 00:00:00 and 1970-01-01 00:00:00
